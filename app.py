"""
Peta Digital UNJ — Sistem Informasi Geografis Kampus
Aplikasi web interaktif berbasis Streamlit + Folium + SQLite.
Seluruh perhitungan rute berjalan 100% offline (graf lokal), tanpa
bergantung pada API peta pihak ketiga saat runtime.
"""
...
# Batas area kampus UNJ (bounding box OSM + buffer 300m)
_CAMPUS_LAT = (-6.1985, -6.1910)
_CAMPUS_LON = (106.8745, -6.8830)
...
def _is_near_campus(lat: float, lon: float, radius_m: float = 800) -> bool:
    ...
    dist = 2 * 6_371_000 * math.asin(math.sqrt(a))
...
@st.cache_resource
def get_osm_network():
    """Muat graf OSM sekali saja (dipakai ulang antar rerun). None jika belum tersedia."""
...
def load_routing_gedung(_conn):
    """Termasuk waypoint jalur ('WP_%') — khusus untuk membangun graf rute."""
...
with st.sidebar:
    st.markdown(
        ...
        '<div style="font-size:1rem;font-weight:700;color:#E8F0FE;">Peta Digital UNJ</div>'
        '<div style="font-size:0.7rem;color:#6A82A8;">Navigasi Kampus Interaktif</div>'
        '</div>',
        unsafe_allow_html=True,
    )
    st.divider()
    ...
    selected_kat = st.multiselect(
        "Tampilkan kategori:", options=all_kat, default=[], label_visibility="collapsed",
    )

    st.markdown('<div class="sidebar-section">🔍 Pencarian Lokasi</div>', unsafe_allow_html=True)
    search_query = st.text_input(
        "Cari nama gedung, fasilitas, atau deskripsi:",
        placeholder="Contoh: rektorat, kantin, parkir...",
    ...
    tile_mode = st.selectbox(
        "Tile layer:", options=list(map_builder.TILE_LAYERS.keys()), index=0,
        label_visibility="collapsed",
    )
    ...
    if _osm_active:
        st.markdown(
            '<div style="font-size:0.72rem;color:#4CD9C0;margin-bottom:6px;">'
            '🛰️ Mode OSM aktif — rute mengikuti jalan/trotoar asli</div>',
            unsafe_allow_html=True,
        )
    else:
        st.markdown(
            '<div style="font-size:0.72rem;color:#8FA3BF;margin-bottom:6px;">'
            '🧩 Mode waypoint offline (jalankan <code>database/build_network.py</code> '
            'untuk rute presisi jalan asli)</div>',
            unsafe_allow_html=True,
        )
    ...
    with col_r2:
        tujuan_nama = st.selectbox("Tujuan:", options=["— pilih —"] + nama_gedung,
                                    key="route_tujuan")
    ...
    if btn_rute:
        if asal_nama == "— pilih —" or tujuan_nama == "— pilih —":
            ...
            if asal_is_gps:
                ...
                # Cek apakah GPS user di dalam/dekat area kampus
                if not _is_near_campus(asal_lat, asal_lon, radius_m=3000):
                    st.warning(
                        f"⚠️ GPS kamu terdeteksi jauh dari kampus UNJ "
                        f"({asal_lat:.4f}, {asal_lon:.4f} — sekitar "
                        f"{routing._haversine(asal_lat, asal_lon, -6.1948, 106.8785)/1000:.1f} km). "
                        "Rute ditampilkan dari pintu masuk kampus."
                    )
                    # Snap ke titik tengah kampus
            ...
            if osm_graph is not None:
                path_coords, jarak = routing.find_shortest_path_osm(
                    osm_graph, asal_lat, asal_lon,
                    tujuan_row["latitude"], tujuan_row["longitude"],
                )
                ...
                path_names = [
                    G.nodes[n]["nama"] for n in path_nodes
                    if not str(G.nodes[n]["nama"]).startswith("WP_")
                ] if path_coords else []
                if path_names:
            ...
            if path_coords:
                ...
                import time
                st.session_state.do_scroll = time.time()
                db.insert_log(conn, "route", st.session_state.sesi_id,
                              detail={"asal": asal_nama, "tujuan": tujuan_nama, "jarak": jarak})
            else:
    ...
    st.divider()
    st.markdown(
        '<div style="font-size:0.68rem;color:#3A5278;text-align:center;line-height:1.6;">'
        'Data koordinat berdasarkan pemetaan kampus UNJ Rawamangun.<br/>',
        unsafe_allow_html=True,
    )

# ── Main Tabs ─────────────────────────────────────────────────────────────

tab_peta, tab_dashboard, tab_data = st.tabs(
    ["🗺  Peta Interaktif", "📊  Dashboard Analitik", "📋  Data & Ekspor"]
)
...
with tab_peta:
    if selected_kat:
        ...
        display_df = gedung_all.iloc[0:0]  # kosong, tapi kolom tetap ada
    ...
    if search_query.strip():
        ...
        # Cari di SEMUA gedung (bukan hanya yang difilter kategori)
        mask_gedung = (
            gedung_all["nama_gedung"].str.lower().str.contains(sq, na=False) |
            gedung_all["deskripsi"].str.lower().str.contains(sq, na=False) |
            gedung_all["kategori"].str.lower().str.contains(sq, na=False) |
            gedung_all["fakultas_unit"].str.lower().str.contains(sq, na=False)
        )
        matched = gedung_all[mask_gedung]

        # Cari juga di fasilitas -> arahkan ke gedung induknya (dari semua gedung)
        ...
        # Gabungkan hasil pencarian ke display_df agar tampil di peta
        # walau kategorinya tidak ada di filter aktif
        if not matched.empty:
            display_df = pd.concat([display_df, matched]).drop_duplicates(subset="id_gedung")
            db.insert_log(conn, "search", st.session_state.sesi_id,
                          detail={"query": search_query, "results": len(matched)})

        if highlight_ids:
            badges = " ".join(
                f'<span class="search-badge">📍 {n}</span>' for n in matched["nama_gedung"].head(6)
            )
            extra = f'<span class="search-badge">+{len(highlight_ids)-6} lainnya</span>' \
                    if len(highlight_ids) > 6 else ""
            st.markdown(
                f'<div style="padding:0.4rem 0;margin-bottom:0.5rem;">'
                f'🔍 Ditemukan <b style="color:#3FE0D8">{len(highlight_ids)}</b> lokasi: '
                f'{badges}{extra}</div>',
                unsafe_allow_html=True,
            )
        else:
    ...
    if st.session_state.get("do_scroll", 0) > 0:
        ...
        # Jangan reset do_scroll di sini karena reruns bisa hilang scrollnya, biarkan ID unik yang mentrigger

    folium_map = map_builder.build_map(
        gedung_df=display_df,
        fasilitas_df=fasilitas_all,
        tile_mode=tile_mode,
    ...
    if map_data and map_data.get("last_object_clicked_tooltip"):
        ...
        if tooltip_text != st.session_state.last_map_click:
            ...
            nama_klik = tooltip_text.split("<br")[0].replace(
                "<b style='font-family:Segoe UI'>", "").replace("</b>", "").strip()
            row = gedung_all[gedung_all["nama_gedung"].str.startswith(nama_klik)]
            gid = int(row.iloc[0]["id_gedung"]) if not row.empty else None
            db.insert_log(conn, "view_marker", st.session_state.sesi_id,
                          id_gedung=gid, detail={"tooltip": tooltip_text[:80]})
            if not row.empty:
    ...
    # ── Panel Navigasi (Streamlit-native, selalu bisa diklik) ─────────────
    if st.session_state.selected_building:
        bname = st.session_state.selected_building
        st.markdown(
            f'<div style="background:linear-gradient(135deg,#111F3A,#1A2F50);'
            f'border:1px solid #2A4570;border-radius:12px;padding:12px 16px;'
            f'margin-top:8px;display:flex;align-items:center;justify-content:space-between;">'
            f'<span style="color:#E8F0FE;font-weight:700;font-size:14px;">🏛 {bname}</span>'
            f'<span style="color:#8FA3BF;font-size:12px;">Klik tombol di bawah untuk navigasi</span>'
            f'</div>',
            unsafe_allow_html=True,
        )
        col_nav_a, col_nav_b = st.columns([4, 1])
        with col_nav_a:
            if st.button(f"🧭 Navigasi ke {bname} dari GPS saya", key="btn_nav_popup",
                ...
                if not st.session_state.gps_coords:
                    ...
                    if not _is_near_campus(asal_lat, asal_lon, radius_m=3000):
                        st.warning(
                            f"⚠️ GPS kamu terdeteksi jauh dari kampus UNJ ({asal_lat:.4f}, {asal_lon:.4f}). "
                            "Rute akan dihitung dari pintu masuk kampus terdekat."
                        )
                        asal_lat, asal_lon = -6.1948, 106.8785
                    ...
                    if osm_graph is not None:
                        ...
                        G = routing.build_graph(edges_all, routing_gedung_all)
                        nearest_node = routing.nearest_graph_node(G, asal_lat, asal_lon)
                        path_coords, path_nodes, jarak = routing.find_shortest_path(
                            G, nearest_node, tujuan_id)
                        if path_coords:
                            path_coords = [(asal_lat, asal_lon)] + path_coords
                        path_names = [
                            G.nodes[n]["nama"] for n in (path_nodes or [])
                            if not str(G.nodes[n]["nama"]).startswith("WP_")
                        ]
                        if path_names:
                    ...
                    if path_coords:
                        ...
                        st.session_state.route_asal = GPS_OPTION_LABEL
                        db.insert_log(conn, "route", st.session_state.sesi_id,
                                      detail={"asal": "GPS", "tujuan": bname, "jarak": jarak})
    ...
    st.markdown(
        f'<div class="map-stats-bar">'
        f'<span>📍 <b style="color:#E8F0FE">{total_shown}</b> gedung ditampilkan</span>'
        f'<span>🏷 <b style="color:#E8F0FE">{len(selected_kat)}</b> kategori aktif</span>'
        f'<span>🧭 Rute: <b style="color:#E8F0FE">{"Aktif" if route_coords else "Tidak aktif"}</b></span>'
        f'</div>',
        unsafe_allow_html=True,
    )
...
with tab_dashboard:
    stats = db.get_total_stats(conn)
    c1, c2, c3, c4 = st.columns(4)
    with c1:
        st.markdown(
            f'<div class="metric-card teal"><span class="metric-icon">📍</span>'
            f'<div class="metric-value">{stats["total_gedung"]}</div>'
            f'<div class="metric-label">Total Gedung</div></div>', unsafe_allow_html=True)
    with c2:
        st.markdown(
            f'<div class="metric-card gold"><span class="metric-icon">🧩</span>'
            f'<div class="metric-value">{stats["total_fasilitas"]}</div>'
            f'<div class="metric-label">Fasilitas</div></div>', unsafe_allow_html=True)
    with c3:
        st.markdown(
            f'<div class="metric-card purple"><span class="metric-icon">🧭</span>'
            f'<div class="metric-value">{stats["total_rute"]}</div>'
            f'<div class="metric-label">Jalur Rute</div></div>', unsafe_allow_html=True)
    with c4:
        st.markdown(
            f'<div class="metric-card green"><span class="metric-icon">📊</span>'
            f'<div class="metric-value">{stats["total_log"]}</div>'
            f'<div class="metric-label">Total Aktivitas</div></div>', unsafe_allow_html=True)
    ...
    with col_a:
        kat_visit = db.get_kategori_visit_count(conn)
        st.plotly_chart(analytics.chart_kategori_terpopuler(kat_visit),
                         width="stretch", config={"displayModeBar": False})
    with col_b:
        dist_df = db.get_aksi_distribution(conn)
        st.plotly_chart(analytics.chart_distribusi_aksi(dist_df),
                         width="stretch", config={"displayModeBar": False})
    st.markdown('</div>', unsafe_allow_html=True)

    st.markdown('<div class="content-card">', unsafe_allow_html=True)
    tren_df = db.get_aksi_trend(conn)
    st.plotly_chart(analytics.chart_tren_pencarian(tren_df),
                     width="stretch", config={"displayModeBar": False})
    st.markdown('</div>', unsafe_allow_html=True)
    ...
    with col_c:
        fig_scatter = analytics.chart_heatmap_lokasi(gedung_all)
        st.plotly_chart(fig_scatter, width="stretch", config={"displayModeBar": False})
    with col_d:
        fas_kat_df = fasilitas_all.groupby("kategori_fasilitas").size().reset_index(name="jumlah") \
            if not fasilitas_all.empty else pd.DataFrame()
        st.plotly_chart(analytics.chart_fasilitas_per_kategori(fas_kat_df),
                         width="stretch", config={"displayModeBar": False})
    st.markdown('</div>', unsafe_allow_html=True)

    st.markdown(
        '<div class="section-header"><div class="bar"></div><h3>📋 Log Aktivitas Terbaru</h3></div>',
        unsafe_allow_html=True,
    )
    st.markdown('<div class="content-card">', unsafe_allow_html=True)
    log_limit = st.slider("Tampilkan", 10, 200, 50, 10, key="log_limit")
    log_df = db.get_recent_logs(conn, limit=log_limit)

    if not log_df.empty:
        col_rename = {
            "id_log": "ID", "timestamp": "Waktu", "jenis_aksi": "Jenis Aksi",
            "nama_gedung": "Gedung", "id_sesi": "Sesi", "detail_json": "Detail",
        }
        log_display = log_df.rename(columns=col_rename)
        st.dataframe(
            log_display, width="stretch", hide_index=True,
            column_config={
                "Waktu": st.column_config.TextColumn(width="medium"),
                "Jenis Aksi": st.column_config.TextColumn(width="small"),
                "Gedung": st.column_config.TextColumn(width="large"),
...
with tab_data:
    st.markdown(
        '<div class="section-header"><div class="bar"></div>'
        '<h3>🗄 Database Gedung Kampus UNJ</h3></div>',
        unsafe_allow_html=True,
    )
    ...
    display_cols = ["foto", "id_gedung", "nama_gedung", "kategori", "fakultas_unit",
                     "latitude", "longitude", "jam_operasional", "deskripsi"]
    st.markdown('<div class="content-card">', unsafe_allow_html=True)
    st.dataframe(
        data_df[[c for c in display_cols if c in data_df.columns]],
        width="stretch", hide_index=True,
        column_config={
            "foto": st.column_config.ImageColumn("Foto", width="small"),
            "id_gedung": st.column_config.NumberColumn("ID", width="small"),
            "nama_gedung": st.column_config.TextColumn("Nama Gedung", width="large"),
            "kategori": st.column_config.TextColumn("Kategori", width="medium"),
            "fakultas_unit": st.column_config.TextColumn("Fakultas/Unit", width="medium"),
            "latitude": st.column_config.NumberColumn("Lat", format="%.6f", width="small"),
            "longitude": st.column_config.NumberColumn("Lon", format="%.6f", width="small"),
            "jam_operasional": st.column_config.TextColumn("Jam Operasional", width="medium"),
    ...
    st.markdown(
        '<div class="section-header"><div class="bar"></div><h3>🧩 Database Fasilitas</h3></div>',
        unsafe_allow_html=True,
    )
    ...
    st.markdown('<div class="content-card">', unsafe_allow_html=True)
    st.dataframe(
        fasilitas_display[["foto", "id_fasilitas", "nama_fasilitas", "kategori_fasilitas",
                            "unit_pengguna", "nama_gedung", "deskripsi"]],
        width="stretch", hide_index=True,
        column_config={
            "foto": st.column_config.ImageColumn("Foto", width="small"),
            "id_fasilitas": st.column_config.NumberColumn("ID", width="small"),
            "nama_fasilitas": st.column_config.TextColumn("Nama Fasilitas", width="medium"),
            "kategori_fasilitas": st.column_config.TextColumn("Kategori", width="small"),
            "unit_pengguna": st.column_config.TextColumn("Unit/Fakultas", width="medium"),
            "nama_gedung": st.column_config.TextColumn("Gedung Terkait", width="medium"),
            "deskripsi": st.column_config.TextColumn("Deskripsi", width="large"),
        },
    )
    st.markdown('</div>', unsafe_allow_html=True)
    ...
    st.markdown(
        '<div class="section-header"><div class="bar"></div><h3>💾 Ekspor Data</h3></div>',
        unsafe_allow_html=True,
    )
    col_e1, col_e2, col_e3, col_e4 = st.columns(4)
    with col_e1:
        st.download_button("⬇ gedung.csv", data=export.gedung_to_csv_bytes(gedung_all),
                            file_name="gedung.csv", mime="text/csv", width="stretch")
    with col_e2:
        geojson_str = export.gedung_to_geojson(gedung_all)
        st.download_button("⬇ gedung.geojson", data=export.geojson_to_bytes(geojson_str),
                            file_name="gedung.geojson", mime="application/geo+json",
                            width="stretch")
    with col_e3:
        st.download_button("⬇ fasilitas.csv", data=export.fasilitas_to_csv_bytes(fasilitas_all),
                            file_name="fasilitas.csv", mime="text/csv", width="stretch")
    with col_e4:
        log_df_exp = db.get_recent_logs(conn, limit=1000)
        if st.download_button("⬇ log_aktivitas.csv", data=export.log_to_csv_bytes(log_df_exp),
                               file_name="log_aktivitas.csv", mime="text/csv",
                               width="stretch"):
...
st.markdown(
    ...
    'Peta Digital UNJ &nbsp;|&nbsp; Sistem Informasi Geografis Kampus &nbsp;|&nbsp; '
    'Data koordinat: Pemetaan Manual Kampus UNJ Rawamangun, Jakarta Timur'
    '</div>',
    unsafe_allow_html=True,
)
